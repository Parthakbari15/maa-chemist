      ...
      <div class="card">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ salesStore.sales.length }}</p>
          </div>
        </div>
      </div>
      <div class="card">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-bold text-gray-900">₹{{ totalSalesAmount.toFixed(2) }}</p>
          </div>
        </div>
      </div>
      <div class="card">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="sale in filteredSales" :key="sale.id" class="hover:bg-gray-50 cursor-pointer" @click="viewSaleDetail(sale)">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ sale.billNo }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ sale.billDate }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ sale.toName }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <span class="px-2 py-1 text-xs rounded-full" :class="sale.paymentType === 'Cash' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'">
                  {{ sale.paymentType }}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ sale.items?.length || 0 }} items</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{{ sale.grandTotal?.toFixed(2) || '0.00' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <button @click.stop="editSale(sale)" class="text-indigo-600 hover:text-indigo-900 mr-3">Edit</button>
                <button @click.stop="deleteSale(sale)" class="text-red-600 hover:text-red-900">Delete</button>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-7xl sm:w-full">
          <form @submit.prevent="saveSale">
            <div class="bg-white px-6 pt-5 pb-4 sm:p-8 sm:pb-4 max-h-screen overflow-y-auto">
              <!-- Maa Chemist Header -->
              <div class="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-4 rounded-lg mb-6">
                <h3 class="text-xl font-bold">Maa Chemist</h3>
                <p class="text-purple-100 text-sm">Medical & General Store | GSTIN: 07AAAPM1234C1ZY</p>
              </div>
              ...
              <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-gray-500">
                  {{ editingSale ? 'Update sale information' : 'Create a new sale bill' }}
                </p>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                ...
                <div>
                  ...
                  <select v-model="formData.paymentType" class="input w-full px-3 py-2" required style="min-height: 40px; font-size: 14px;">
                  ...
                  <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                      <tr>
                        ...
                        <th class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase">Disc %</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr v-for="(item, index) in formData.items" :key="index">
                        ...
                        <td class="px-2 py-2 relative" style="position: relative;">
                          <input
              ...
              <div class="border-t pt-4 mt-6">
                <div class="flex justify-end">
                  <div class="text-right">
                    ...
                    <p class="text-3xl font-bold text-gray-900">₹{{ grandTotal.toFixed(2) }}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Are you sure you want to delete sale bill {{ deleteConfirmation.billNo }}? This action cannot be undone.
                  </p>
                </div>
              </div>
            </div>
...
const totalSalesAmount = computed(() => {
  return salesStore.sales.reduce((total, sale) => total + (sale.grandTotal || 0), 0)
})
...
const findExistingProduct = (productName) => {
  ...
  for (const sale of sales) {
    if (sale.productName && sale.productName.toLowerCase() === productName.toLowerCase()) {
      return {
    ...
    if (sale.items && sale.items.length > 0) {
      for (const item of sale.items) {
        if (item.product && item.product.toLowerCase() === productName.toLowerCase()) {
          return {
...
const filteredSuggestions = computed(() => {
  ...
  const suggestions = productSuggestions.value.filter(product =>

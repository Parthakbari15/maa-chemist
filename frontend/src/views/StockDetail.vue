              ...
              <div>
                ...
                <div class="flex items-center space-x-2">
                  <span
                    ...
                    :class="getStockClass(stock.availableStock)"
                  >
          ...
          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-4">
              <div>
                ...
                <div class="flex items-center space-x-2">
                  <span class="text-2xl font-bold text-red-600"
                    >₹{{ (stock.purchaseValue || 0).toFixed(2) }}</span
                  >
                </div>
              </div>
            ...
            <div class="space-y-4">
              <div>
                ...
                <div class="flex items-center space-x-2">
                  <span class="text-2xl font-bold text-green-600"
                    >₹{{ (stock.salesValue || 0).toFixed(2) }}</span
                  >
                </div>
              </div>
            ...
            <div>
              ...
              <div class="w-full bg-gray-200 rounded-full h-3">
                <div
                  ...
                  :style="{
                    width: `${Math.min(100, (stock.availableStock / Math.max(stock.totalPurchased, 1)) * 100)}%`,
                  }"
                ></div>
              </div>
            </div>
...
const recentPurchases = computed(() => {
  ...
  const purchases = purchaseStore
    .getPurchasesByProduct(stock.value.productName)
...
const recentSales = computed(() => {
  ...
  const sales = salesStore
    .getSalesByProduct(stock.value.productName)
    .sort((a, b) => new Date(b.date) - new Date(a.date))
    .slice(0, 5);

  ...
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center space-x-2">
        ...
        >
          <RefreshCw :class="['h-4 w-4', stockStore.loading && 'animate-spin']" />
          <span>Refresh</span>
        </button>
        <button
      ...
      <div class="card bg-blue-50 border-blue-200">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xl font-bold text-blue-900">{{ stockStore.stockData.length }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="card bg-yellow-50 border-yellow-200">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xl font-bold text-yellow-900">{{ stockStore.lowStockItems.length }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="card bg-red-50 border-red-200">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xl font-bold text-red-900">{{ stockStore.outOfStockItems.length }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="card bg-gradient-to-r from-blue-500 to-blue-600 text-white">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold">${{ stockStore.totalStockValue.toFixed(2) }}</p>
          </div>
      ...
      <div class="card bg-gradient-to-r from-green-500 to-green-600 text-white">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold">{{ stockStore.totalStockQuantity }}</p>
          </div>
          <div class="bg-white bg-opacity-20 rounded-lg p-3">
      ...
      <div class="card bg-gradient-to-r from-purple-500 to-purple-600 text-white">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold">${{ stockStore.totalProfit.toFixed(2) }}</p>
          </div>
          <div class="bg-white bg-opacity-20 rounded-lg p-3">
    ...
    <div v-if="hasAlerts" class="space-y-4">
      ...
      <!-- Out of Stock -->
      <div v-if="stockStore.outOfStockItems.length > 0" class="card border-l-4 border-red-500 bg-red-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          ...
          >
            <span class="text-sm font-medium text-red-900">{{ item.productName }}</span>
            <button
      ...
      <!-- Low Stock -->
      <div v-if="stockStore.lowStockItems.length > 0" class="card border-l-4 border-yellow-500 bg-yellow-50">
        <div class="flex items-center space-x-3 mb-3">
          <AlertTriangle class="h-5 w-5 text-yellow-600" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          ...
          >
            <div class="flex items-center justify-between">
              <span class="text-sm font-medium text-yellow-900">{{ item.productName }}</span>
              <span class="text-xs text-yellow-700 font-medium">{{ item.availableStock }} left</span>
            </div>
            <div class="mt-1">
              <div class="w-full bg-yellow-200 rounded-full h-1">
                <div
                  class="bg-yellow-500 h-1 rounded-full"
                  :style="{ width: `${Math.min(100, (item.availableStock / 10) * 100)}%` }"
                ></div>
              </div>
            </div>
          </div>
      ...
      <!-- Expiring Soon -->
      <div v-if="stockStore.expiringStockItems.length > 0" class="card border-l-4 border-orange-500 bg-orange-50">
        <div class="flex items-center space-x-3 mb-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          ...
          >
            <div class="flex items-center justify-between">
              <span class="text-sm font-medium text-orange-900">{{ item.productName }}</span>
              <span class="text-xs text-orange-700">{{ formatDate(item.latestExpiryDate) }}</span>
            </div>
    ...
    <div class="card">
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-semibold text-gray-900">Current Stock Levels</h2>
        <div class="flex items-center space-x-2">
          <input
            v-model="searchQuery"
            type="text"
        ...
        <table class="table">
          ...
          <tbody>
            ...
            >
              ...
              <td class="table-cell">
                <span v-if="stock.latestExpiryDate" :class="getExpiryClass(stock.latestExpiryDate)">
    ...
    <div v-if="selectedStock" class="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div class="flex min-h-screen items-center justify-center p-4">
        <div class="relative bg-white rounded-lg shadow-xl max-w-2xl w-full animate-bounce-in">
          <div class="bg-blue-600 text-white p-6 rounded-t-lg">
            <h2 class="text-xl font-bold">Stock Details - {{ selectedStock.productName }}</h2>
          </div>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-2">
                  <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ selectedStock.totalPurchased }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Purchase Value:</span>
                    <span class="font-medium">${{ selectedStock.purchaseValue.toFixed(2) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Avg Purchase Price:</span>
                    <span class="font-medium">
                      ${{ selectedStock.totalPurchased > 0 ? (selectedStock.purchaseValue / selectedStock.totalPurchased).toFixed(2) : '0.00' }}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-2">
                  <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ selectedStock.totalSold }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Sales Value:</span>
                    <span class="font-medium">${{ selectedStock.salesValue.toFixed(2) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Avg Sale Price:</span>
                    <span class="font-medium">
                      ${{ selectedStock.totalSold > 0 ? (selectedStock.salesValue / selectedStock.totalSold).toFixed(2) : '0.00' }}
                    </span>
                  </div>
                </div>
              </div>
...
const filteredStock = computed(() => {
  ...
  switch (stockFilter.value) {
    case 'instock':
      ...
      break
    case 'lowstock':
      stock = stock.filter(item => item.availableStock > 0 && item.availableStock <= 10)
      break
    case 'outofstock':
...
const hasAlerts = computed(() => {
  return stockStore.outOfStockItems.length > 0 ||
...
const getStatusBadgeClass = (stock) => {
  if (stock.availableStock <= 0) return 'bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full'
  if (stock.availableStock <= 10) return 'bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded-full'
  if (stock.isExpiringSoon) return 'bg-orange-100 text-orange-800 text-xs px-2 py-1 rounded-full'
...
const exportStockData = () => {
  const csvContent = [
    ['Product Name', 'Total Purchased', 'Total Sold', 'Available Stock', 'Purchase Value', 'Sales Value', 'Latest Expiry Date', 'Status'],
